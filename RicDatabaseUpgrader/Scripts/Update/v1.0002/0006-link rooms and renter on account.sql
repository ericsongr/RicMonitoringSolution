...
--RENTER
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Renters' AND COLUMN_nAME = 'AccountId')
BEGIN
	ALTER TABLE Renters ADD AccountId INT NULL
...
IF EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Renters' AND COLUMN_nAME = 'AccountId')
BEGIN
	DECLARE @accountId INT
	
	SELECT TOP 1 @accountId = ID FROM Accounts ORDER BY id DESC

	UPDATE Renters SET AccountId = @accountId

	ALTER TABLE Renters ALTER COLUMN AccountId INT NOT NULL 
END
GO